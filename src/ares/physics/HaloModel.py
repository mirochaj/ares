...
class HaloModel(HaloMassFunction):
        ...
        if type(k) == np.ndarray:
            ...
            for _k in k:
                _integ1, _integ2 = self._integrate_over_prof(_k, iz,
                    ...
                    term, mmax1, mmax2, weight_by_mass=weight_by_mass)
    ...
    def _integrate_over_prof(self, k, iz, prof1, prof2, lum1, lum2, mmin1,
        ...
        rho_bar = self.cosm.mean_density0 # [Msun/cMpc^3]
        dndlnm = self.tab_dndlnm[iz]

        # If no lum provided, assumes we're doing matter field
        if weight_by_mass:
        ...
        if term == 1:
            ...
            integral1 = np.trapezoid(integrand1[ok*ok1==1], x=np.log(self.tab_M[ok*ok1==1]),
                axis=0)

            integrand2 = dndlnm * focc2 * weight2 * p2 * bias / norm2
            integral2 = np.trapezoid(integrand2[ok*ok2==1], 
                x=np.log(self.tab_M[ok*ok2==1]), axis=0)
            corr2 = 0
    ...
    def get_ps_1h(self, z, k=None, prof1=None, prof2=None, lum1=None, lum2=None,
        mmin1=None, mmin2=None, focc1=1, focc2=1, mmax1=np.inf, mmax2=np.inf, ztol=1e-3):
        """
        ...
        integ1, none = self._get_ps_integrals(k, iz, prof1, prof2,
            lum1, lum2, mmin1, mmin2, focc1, focc2, 1, mmax1, mmax2)

        return integ1

    def get_ps_2h(self, z, k=None, prof1=None, prof2=None, lum1=None, lum2=None,
        mmin1=None, mmin2=None, mmax1=np.inf, mmax2=np.inf, 
        focc1=1, focc2=1, weight_by_mass=True, ztol=1e-3):
        """
    ...
    def get_ps_shot(self, z, k=None, lum1=None, lum2=None, mmin1=None, mmin2=None,
        ...
        shot = np.trapezoid(integrand[ok==1], x=np.log(self.tab_M[ok==1]), axis=0)
