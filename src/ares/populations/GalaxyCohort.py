...
class GalaxyCohort(GalaxyAggregate):
    ...
    def get_size(self, z, Ms=None):
        """
        Return the half-light radius in kpc for galaxy at redshift `z` with stellar mass `Ms`.
        """
        if Ms is None:
            Mh = self.halos.tab_M
    ...
    def get_light_fraction_in_aperture(self, z, ap=2.):
        """
        Figure out how much light comes from central region of resolved source.

        .. note :: This requires `pop_msr` and `pop_profile_info`.
        ...
        arcsec_per_pmpc = 60 * self.cosm.get_angle_from_length_proper(
            z, 1.
        )
        ...
        # Axis ratios first
        ba_loc, ba_scale = self.pf['pop_profile_info'][f'{pop_s}_ba'][key]
        ellip_loc = 1 - ba_loc

        # Now Sersic indices
        n_loc, n_scale = self.pf['pop_profile_info'][f'{pop_s}_n'][key]

        ##
        # Have to do this in 2-D to be precise. 
    ...
    def get_ssfr_obs(self, z):
        ...
        return 10**(np.log10(sfr / Ms) + offset)

    def get_ssfr_sys(self, **kwargs):
        """
        Return the log10(systematic offset) between observed and true specific
        star formation rates. This is uniquely determined by the combination
        of biases on stellar masses and star formation rates.
        """

        if not (self.is_biased_mass or self.is_biased_sfr):
            return 0.0

        if self.pf['pop_sys_method'] == 'b13':
            return self.pf['pop_sys_sfr_now'] \
                * np.exp(-(kwargs['z'] - 2)**2 / 2.)

        # General case
        dMst = self.get_mstell_sys(**kwargs)
        dsfr = self.get_sfr_sys(**kwargs)

        return dsfr - dMst

    def get_mstell_sys(self, **kwargs):
        """
        Return the log10(systematic offset) between observed and true stellar
        masses.
        """
        ...
        return self.pf['pop_sys_mstell_now'] + self.pf['pop_sys_mstell_a'] * (1. - a)

    def get_mstell_obs(self, **kwargs):
        """
        Return the "observed" stellar mass for all galaxies vs. halo mass.
        ...
        that characterize potential errors in measured stellar masses. The
        offset itself can be retrieved via the `get_mstell_sys` method above.
    ...
    def get_sfr_sys(self, **kwargs):
        ...
        # the `pop_sys_sfr_now` actually controls the sSFR.
        if self.pf['pop_sys_method'] == 'b13':
            return self.get_mstell_sys(**kwargs) \
                + self.pf['pop_sys_sfr_now'] * np.exp(-(kwargs['z'] - 2)**2 / 2.)
        else:
            a = 1. / (1. + kwargs['z'])
            return self.pf['pop_sys_sfr_now'] + self.pf['pop_sys_sfr_a'] * (1. - a)


    def get_sfr_obs(self, **kwargs):
        """
        Return the "observed" SFR for all galaxies vs. halo mass.
    ...
    def get_emissivity(self, z, x=None, band=None, units='eV',
        ...
        if self.is_emissivity_scalable:
            ...
            if self.pf['pop_scatter_sfh'] == self.pf['pop_scatter_sfr'] == 0:
                ...
                integ1 = L1 * self.halos.tab_dndlnm[iz,:] \
                    * self.tab_focc[iz,:]
    ...
    def get_mf(self, z, bins=None, units='dex', mass='stellar', use_tabs=True):
        """
        Return stellar mass function, dn/dlog10(Mstell).
        ...
        if self.is_user_smhm:
            ...
            if mass == 'stellar':
                ...
                dlog10mdlog10M = np.diff(logMh_e) / np.diff(np.log10(Mg_e))
            ...
            if self.is_central_pop:
                if use_tabs:
                    ...
                    dndlnm = dndlnm * self.get_focc(z=z, Mh=self.halos.tab_M)
            ...
            # Extra step if we're dealing with satellites
                ...
                # First, get the total number of satellites as a function
                # of subhalo mass
                dndlnm_sat = np.zeros_like(self.halos.tab_M)
                for i, Msat in enumerate(self.halos.tab_M):
                    ...
                    integrand = dndlnm * focc[i] * fsurv[i] \
                        * self.halos.tab_dndlnm_sub[:,i]
    ...
    def get_pdf_mstell(self, z, log10M=None):
        ...
        if log10M is None:
            lnM = np.log(10**np.log10(self.get_mstell_obs(z=z, Mh=self.halos.tab_M)))
        else:
...
#
    ...
    def get_number_counts(self, bins, zmin=0, zmax=10, x=1600.,
        ...
        volume_density : bool
            If True, returned number density is per deg^2, if False, is in 
            per cMpc^3 units.

        Returns
        -------
        Number counts per mag per deg^2 (or cMpc^-3 if volume_density=True)
        in provided `bins`.
        ...
        assert np.allclose(np.diff(dmag), 0), \
...
#
    ...
    def get_lf(self, z, bins=None, use_tabs=True,
        ...
        if use_mags:
            ...
            if not np.any(ok):
                ...
                xgt0 = np.logical_and(_x_ > 0, ok==1)
                phi_of_x = np.interp(bins, _x_[xgt0==1], phi[xgt0==1], 
                    left=0, right=0)
                phi_of_x = np.ma.array(phi_of_x)
    ...
    def get_bias(self, z, limit, wave=1600., cut_in_mass=False, absolute=False,
        ...
        if cut_in_flux:
            ...
            if type(limit) in [list, tuple, np.ndarray]:
                lo, hi = limit
                ok = np.logical_and(mags >= lo, mags < hi)
            else:
    ...
    def get_lum_sat_tot(self, z, Lsat, use_tabs=True, selection_criteria=None):
        """
        ...
        if use_tabs:
            ...
            focc = self.get_focc(z=z, Mh=self.halos.tab_M)
            if type(focc) in numeric_types:
                focc = np.ones_like(self.halos.tab_M) * focc
            fsurv = self.get_fsurv(z=z, Mh=self.halos.tab_M)
            if type(fsurv) in numeric_types:
                fsurv = np.ones_like(self.halos.tab_M) * fsurv
    ...
    def _get_lum_stellar_pop(self, z, x=1600, use_tabs=True,
        ...
        total_sat=True, separate_lines=False, selection_criteria=None):
        """
                ...
                if self.is_central_pop or \
                  (self.is_satellite_pop and (not total_sat)) or self.is_diffuse:
                    ...
                    if self.pf['pop_ihl'] is not None:
                        ...
                        if self.pf['pop_ihl_definition'] == 1:
                            ihl_lfrac = (fihl / (1. - fihl))
                            _Lh_ *= ihl_lfrac
                        elif self.pf['pop_ihl_definition'] == 2:
                            _Lh_ *= fihl
                        else:
    ...
    def _get_lum_from_tab(self, z, Ms, x=1600, band=None, window=1,
        ...
        # Same deal if requested z > tabulated range
            ...
            kludge = np.interp(np.log10(Ms), ltab_M, lum[-1,:],
                left=0, right=0)
        else:
            ...
            kludge1 = np.interp(np.log10(Ms), ltab_M, lum[ilo,:],
                left=0, right=0)
    ...
    def get_lum(self, z, x=1600, use_tabs=True,
        ...
        if self.pf['pop_lum_func'] is not None:
            ...
            L = self._get_lum_stellar_pop(z, x=x, use_tabs=use_tabs,
                ...
                separate_lines=separate_lines, selection_criteria=selection_criteria)
            if separate_lines:
                Lh, Lh_l = L
            else:
                Lh = L
    ...
    def get_ihl_suppression(self, z, Mh):
        ...
        # subhalos in them that have been masked out.
            ...
            # The value of this parameter is a list of two-element tuples,
            # each element containing:
            # (1) the occupation fraction, i.e., the fraction of (sub)halos that
            # host a satellite, and (2) the fraction of those satellites bright
            # enough to be masked out. It's a list because we can have
            # different kinds of satellites.
            # All of these quantities are (self.halos.tab_z, self.halos.tab_M)

            # To determine IHL suppression, we're going to compute the total
            # projected area that's masked out, i.e., the integral over the
            # number of sources * their projected size. For now we'll ignore
            # the fact that we're probably masking out more "core IHL" since
            # massive subhalos are likely centrally concentrated.
            # We're also hard-coding a reasonable size in pixels for now.
            ...
            Rvir_ang = [self.cosm.get_angle_from_length_comoving(z, RR) * 60 \
                for RR in Rvir_mpc]

            # Area of central halos vs. mass in arcsec**2
            area_per_halo = 4 * np.pi * np.array(Rvir_ang)**2

            # Assume for now that subhalos are all the same size
            # (measured in pixels for now)
            area_per_subh = self.pf['pop_ihl_mask_pix']**2

            #
            _flost = num_mask * area_per_subh / area_per_halo
            flost = np.minimum(_flost, 1)
    ...
    def tab_fsel(self):
        """
        Array of shape (redshift, mass) containing the fraction of halos
        that satisfy the selection criteria (in self.pf['pop_selection']).
        """
        if not hasattr(self, '_tab_fmask'):
            self._tab_fmask = self.get_galaxy_subsample(self.pf['pop_selection'], 
                logic=self.pf['pop_mask_logic'])
        return self._tab_fmask
    
    def get_lum_cut_from_mag_cut(self, z, selection_criteria):
        assert 'mag' in selection_criteria.keys(), \
            "Must provide 'mag' cut only!"
    ...
    def get_galaxy_subsample(self, selection_criteria=None, return_fraction=True, 
        ...
        if 'mag' in selection_criteria:
            ...
            for element in selection_criteria['mag']:
                if element[0] not in _bands:
                    _bands.append(element[0])

            select_on_multi_band_cut = len(_bands) > 1
...
#
    ...
    def _get_fsel(self, fsel, z=None):
        """
        This secret routine just makes sure that the selection function has
        the right shape for subsequent operations. From the ares.simulations 
        level, `fsel` will be generated over the full (z, Mh) range -- here, 
        we just take a redshift slice if `z` is not None, or return as-is for
        cases where `fsel` is just a number.
        """
        if z is None:
            ones = np.ones(list(self.halos.tab_dndm.shape) + [2])
        else:
            ones = np.ones(list(self.halos.tab_M.shape) + [2])
        ...
        if fsel is None:
            ...
            assert fsel.shape[0] == self.halos.tab_z.size, \
                "Shape of `fsel` is wrong!"
            if z is None:
                _fsel = fsel
            else:
                iz = self.get_zindex(z)
    ...
    def get_num_from_fsel(self, fsel, z=None, volume_density=True, zbin=None):
        ...
        Parameters
        ----------
        fsel : int, float, np.ndarray
            If just a number, it's probably unity, in which case the result
            is just the f_occ-weighted HMF.
            If an array, can be 1-D array (over halo mass), in which case 
            the more general solution with mass-dependent selection will be
            obtained. A 2-D array indicates the user wants the result 
            over the full (z, M) space.
        z : int, float, None
            If provided, indicates a calculation at a single z. If `fsel`
            is provided as a 2-D array we'll first hack out the slice
        ...
        Returns
        -------
        Number density of galaxies in units determined by `volume_density` 
        parameter.
        ...
        if (z is not None):
            ...
            if self.is_central_pop:
                num = np.trapezoid(self.halos.tab_dndlnm[iz,:] 
                    * self.tab_focc[iz,:] * _fsel[:,0], 
                ...
                dndlnm_pre = self.halos.tab_dndlnm[iz][:,None] \
                           ...
                           * focc[None,:] * fsurv[None,:] * _fsel[:,0][None,:]
                dndlnm = np.trapezoid(dndlnm_pre, dx=self.halos.dlnm, axis=0)
    
                num = np.trapezoid(dndlnm, x=self.halos.dlnm)
        ...
        for i, z in enumerate(self.halos.tab_z):
            ...
            if self.is_central_pop:
                ...
                focc = self.tab_focc[i]
                fsurv = self.tab_fsurv[i,:]
                dndlnm_pre = self.halos.tab_dndlnm[i][:,None] \
                           * self.halos.tab_dndlnm_sub \
    ...
    def get_mask(self):
        ...
        if (self.pf['pop_scatter_sfh'] > 0) and (not self.pf['pop_mask_use_adv']):
            return 1 - self.get_galaxy_subsample(self.pf['pop_mask'])
    ...
    def _get_lf_lum(self, z, larr=None, x=1600., window=1, raw=False,
        ...
        Returns
        -------
        Number of galaxies per unit log(luminosity) per unit volume.
    ...
    def get_ps_shot_old(self, z, k, wave1=1600., wave2=1600., raw=False,
        ...
        ##
        # There is no cross-shot contribution for inter-pop
        # cross-terms
    ...
    def get_ps_2h_old(self, z, k, wave1=1600., wave2=1600., 
        field1_is_num=0, field2_is_num=0, fsel1=1, fsel2=1,
        raw=False, nebular_only=False, ztol=1e-3, cross_pop=None):
        """
    ...
    def get_lum_eff(self, z, lum, fsel=None):
        ...
        luminosity fluctuations (at fixed Mh, z) and weighting by 
        the selection function.

        .. note :: At the moment this is used within the halo model
            integrals, but nowhere else. Viewing on its own can be a 
            useful sanity check.
        ...
        Returns
        -------
        Array of effective luminosities with same shape as self.halos.tab_M.
        Will have same units as `lum`.
        """
        if (self.pf['pop_scatter_sfh'] == 0) and \
            (fsel is None or np.all(fsel == 1)):
            return lum
    ...
    def get_ps_sh(self, z, k, wave1, wave2, isnum1=0, isnum2=0, 
        ...
        # just do everything here.
            ...
            # In the cross-shot regime, we ALWAYS need to use the 
            # union of the selection function and mask. However,
            # we keep the use of masking_symmetric essentially as a 
            # null test, since the mask should take L->0 and effectively
            ...
            if self.is_central_pop:
                ...
                if masking_symmetric:
                    dndlnm_pre *= _fsel_union[:,0][None,:]
                else:
                    dndlnm_pre *= _fsel1[:,0][None,:]
                ...
                # Note here that we should integrate over Mmin->infnty
                # for centrals: doesn't matter if central is selected or
                # not, just need satellites. 
                #print('hello', np.any(f[self.halos.tab_M < self.get_Mmin(z)] > 0))
    ...
    def get_ps_3d(self, scale, wave_obs1, wave_obs2=None, fsel1=1, fsel2=1,
        ...
        id2 = pop2.id_num if pop2 is not None else self.id_num
        pb = ProgressBar(scale.shape[0],
            use=use_pb and self.pf['progress_bar'],
            name=f'p(k,{name}; pop #{self.id_num} x pop#{id2})')
        pb.start()
        ...
        for h, _scale_ in enumerate(scale):
            ...
            for i, z in enumerate(zarr):
                if not zok[i]:
                    continue
                integrand[i] = self._get_ps_3d(z, _scale_,
    ...
    def _get_ps_3d(self, z, scale, wave_obs1, wave_obs2, 
        ...
        # Note that ps3d here will have units of luminosity^2 * length^-3 
        # for autos and internal crosses because the matter PS yields a 
        # factor of volume, but then both bias factors carry inverse 
        # volume factors. For shot noise, the length^-3 comes directly
        # from integrating over the HMF.
    ...
    def get_xs_3d(self, scale, wave_obs, zg, scale_units='ell',
        ...
        id2 = pop2.id_num if pop2 is not None else self.id_num
        name += f", pop #{self.id_num} x pop #{id2}"
