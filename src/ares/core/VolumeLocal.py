...
class LocalVolume:
    ...
    def _initialize(self):
        ...
        for src in self.srcs:
            ...
            self.sigma.append((np.ones([self.grid.dims, src.tab_energies_c.size]) \
                   * src.sigma).T)
    ...
    def MultiFreqCoefficients(self, data, absorber, t=None):
        ...
        N = np.ones([self.src.tab_energies_c.size, self.grid.dims]) * self.N[absorber]
        ...
        k_ion_E = np.zeros([self.grid.dims, self.src.tab_energies_c.size])
