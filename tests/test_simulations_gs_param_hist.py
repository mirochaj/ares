...
def test():
    ...
    # Plot temperatures
    mp.grid[1].loglog(z, hydr.cosm.TCMB(z), color='k', ls=':')
    mp.grid[1].loglog(z, Tk(z), color='k', ls='--')
    mp.grid[1].loglog(z, Ts(z), color='k', ls='-')
    ...
    mp.grid[0].legend(loc='lower right', fontsize=10)
