...
class Fluctuations(object):
   ...
   #    return simps(B[iM:] * dndm_b[iM:] * M_b[iM:], x=np.log(M_b[iM:]))
    ...
    def ExpectationValue2pt(self, z, zeta, R, term='ii', R_s=None, R3=None, 
        ...
        if not self.pf['ps_include_ion']:
            if term == 'ii':
                ...
                self._cache_jp_[z][term] = R, Qi * xi_dd, Rzeros, Rzeros
        ...
        for i, sep in enumerate(R):
            ...
            ##    
                ...
                if term == 'id':
                    ...
                    #P2[i] = _P_ii_2[i] * delta_b_bar #- (1. - _P_ii_2[i] - _P_ii_1[i]) * delta_n_bar
        ...
        if (term == 'ii') and (Qi >= 0.5) and self.pf['ps_volfix']:
            ...
            if (Qi >= 0.5) and self.pf['ps_volfix']:
                PT = P1 + P2
            else:
                PT = (1. - Qi) * P1
        elif term in ['idd']:
            ...
            kludge = 0.0
            if not self.pf['ps_use_wick']:
                avg_id = self.ExpectationValue1pt(z, zeta, term='i*d')
                # This is to ensure limiting behavior -> 0 as R -> inf
                kludge += avg_id**2 + 2 * Qi * (avg_id - Qi * delta_b_bar)
    ...
    def CorrelationFunction(self, z, zeta=None, R=None, term='ii', 
        R_s=0.0, R3=0.0, Th=500., Tc=1., Ts=None, k=None, Tk=None, Ja=None):
        """
