...
class GridND(defaultdict):
    ...
    def load(self, fn=None):
        """
        If self.fn was provided, load up the data.
        """
        ...
        self.const_kwargs = {}
        for axis in f['axes']:
            self[axis] = GridAxis(axis, 
                f['axes'][axis].attrs.get('num'), f['axes'][axis].value)
            self.kwargs[axis] = f['axes'][axis].value

        for arg in f['const_kwargs']:
            if type(f['const_kwargs'][arg].value) is np.ndarray:
                self.kwargs[arg] = tuple(f['const_kwargs'][arg].value)
                self.const_kwargs[arg] = tuple(f['const_kwargs'][arg].value)
            else:
                self.kwargs[arg] = f['const_kwargs'][arg].value
                self.const_kwargs[arg] = f['const_kwargs'][arg].value
            
        for key in f.keys():
            if key in ['axes', 'const_kwargs']:
                continue
                
            self[key] = f[key].value
            
        if 'higher_dimensions' in f.keys():
            self.higher_dimensions = f['higher_dimensions'].value
            
        f.close()
        
        self.build(**self.kwargs)
        
    def to_hdf5(self, fn, clobber=False):
        """
        Store current version of class instance to hdf5 file.
        """
        ...
        # Prepare to store axes information
        if 'axes' in f.keys():
            axes = f['axes']
        else:
            axes = f.create_group('axes')
                
        # Loop over attributes, write out axes and data       
        for key in self:
            if key in f.keys():
                if clobber:
                    f.__delitem__(key)
            ...
            if isinstance(self[key], GridAxis):
                ...
                hasvalues = False
                if axis.name in axes.keys():
                    if np.all(axis.values == axes[axis.name]):
                        hasvalues = True
                    else:
                        if self.verbose:
            ...
            # Write dataset
            ds = f.create_dataset(key, data=self[key])
            
        # Write out const_kwargs
        if 'const_kwargs' not in f.keys():
            ck = f.create_group('const_kwargs')
        else:
            ck = f['const_kwargs']
            
        for key in self.const_kwargs:
            try:
